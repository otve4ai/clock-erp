...
## Services vault: управляемые разделы

Миграция `2026-09-20-service-categories-v2` добавляет в изолированную
`services.db` таблицу `service_categories` и сохраняет исходную запись
`2026-08-28-services-vault-v1` в журнале. При обновлении автоматически создаются
разделы «Сайты», «Продажи», «Доставка» и «Инфраструктура»; существующие сервисы,
аккаунты, зашифрованные реквизиты и права не переписываются.

Удаление непустого раздела разрешено только с указанием другого существующего
раздела. Перенос всех сервисов и удаление раздела выполняются одной SQLite
транзакцией. Единственный оставшийся раздел удалить нельзя. `scripts/deploy.sh`
репетирует миграцию на копии, создаёт rollback-backup перед остановленным
production apply и повторно проверяет схему после запуска.
