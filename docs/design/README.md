...
В просмотре и редактировании используется одна сетка: название на всю ширину,
затем пары «Артикул / Цена», «Бренд / Категория», «Остаток / Ячейка».
Модель часов находится в существующем блоке «Дополнительные настройки»,
доступном в обоих режимах; переключатель обязательного ремешка по-прежнему
доступен только при редактировании. Внешние ссылки вынесены в отдельную
вторичную строку под основными действиями. Сохранены компоненты полей, размеры
карточек, адаптивные точки переключения и фотоблок; backend-контракт не меняется.
