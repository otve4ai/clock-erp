...
Форма продажи использует shared ERP modal shell и остаётся единым сценарием
добавления и редактирования. При открытии из общего списка канал не выбирается
автоматически; `Tictactoy`, `Wildberries` и `Amazon` представлены доступным с
клавиатуры segmented control. Смена канала обновляет описание, набор полей и
подпись основной кнопки, сохраняя уже введённые значения.

Товар выбирается существующим server-backed catalog combobox с ограничением
`stock > 0`; каталог не встраивается в HTML. Карточка выбранного товара выводит
фото, название, бренд, категорию, артикул и доступный остаток. Секции «Товар»,
«Продажа», «Данные канала» и «Дополнительно», inline validation, pending и
error-состояния оформлены sales-form контрактом в `erp-components.css`.

На desktop диалог прокручивается только внутри body, а панель действий остаётся
доступной. На mobile диалог занимает экран, поля идут в одну колонку и действия
учитывают safe area. Маршруты, имена полей, payload, idempotency, серверная
validation, списание остатков, возвраты, удаление и архивирование не входят в
визуальный контракт и сохраняют прежнее поведение.
