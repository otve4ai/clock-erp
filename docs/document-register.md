...
| `docs/bitrix_catalog_sync.md` | Runbook синхронизации каталога и статуса сайта | Полный импорт остаётся ручным; status-only сервис, компактный UI и timer 03:00 существуют | `current` | 2026-09-25 | Bitrix catalog | `docs/operations/bitrix-catalog-sync.md` | да | нет | Проверить timer после первого production-развёртывания |
