...
хранит последние 14 ежедневных копий. Воскресная копия одновременно получает
категорию weekly и хранится среди последних 8 недельных копий; копия 1-го числа
получает категорию monthly и хранится среди последних 12 месячных копий. Это один
физический архив, который удаляется только после завершения всех назначенных ему
retention-политик. Ручные архивы создаются только кнопкой ERP в
`/opt/clock-erp-backups/manual` и автоматически не удаляются. Safety-копии
создаются перед восстановлением в `/opt/clock-erp-backups/safety`, проверяются до
изменения production и также не участвуют в автоматической ротации.
...
пишет закрытую metadata с backup ID, timestamp, типом, retention-категориями,
...
commit из metadata. До staging автоматически создаётся и проверяется отдельный
safety backup текущего состояния; его ID сохраняется в операции, а архив остаётся
доступен в UI. Исходный `instance` остаётся частью атомарного swap только на время
операции и удаляется после успешной проверки. При ошибке после swap выполняется одна попытка обратного
