...
if printf '%s\n' "$changed_files" | grep -Eq \
    '^(app/(catalog_db|catalog_migration_steps|schema_migrations|multiwarehouse_migration)\.py|app/catalog(_warehouse)?_schema_manifest\.json|scripts/migration_preflight\.py)$'; then
...
if printf '%s\n' "$changed_files" | grep -Eq \
    '^(app/customer_registry_migrations\.py|scripts/(customer_registry_schema|backfill_customers)\.py)$'; then
