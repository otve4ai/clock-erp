...
class OrdersUiRedesignTest(unittest.TestCase):
    ...
    def test_actions_menu_keyboard_and_responsive_contract(self):
        ...
        self.assertIn("grid-template-columns: 160px minmax(0, 1fr) 148px max-content max-content", self.source)
        self.assertIn("@media (max-width: 900px)", self.source)
        self.assertIn("@media (max-width: 560px)", self.source)
        self.assertIn("@container (max-width: 600px)", self.source)
        self.assertIn("@container (max-width: 460px)", self.source)
        self.assertIn("grid-template-columns: minmax(148px, 1fr) max-content", self.source)
    ...
    def test_order_header_responsive_control_hierarchy(self):
        ...
        self.assertIn(".status-save-state:empty { display:none; }", self.source)
        self.assertIn("flex: 0 0 160px", self.source)
        self.assertIn("min-width: 160px", self.source)
        self.assertIn("grid-template-columns: 160px minmax(0, 1fr) 148px max-content max-content", self.source)
        self.assertIn("padding-inline: 8px", self.source)
        self.assertIn("padding-inline: 6px", self.source)
        self.assertIn("display: contents", self.source)
        self.assertIn("margin-left: 0", self.source)
        self.assertIn("grid-column: 3", self.source)
        self.assertIn("grid-column: 5", self.source)
        self.assertIn("width: 148px", self.source)
        self.assertIn("min-width: 148px", self.source)
        self.assertIn("grid-template-columns: minmax(0, 1fr) 148px max-content max-content", self.source)
