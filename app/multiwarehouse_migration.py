...
def apply_multiwarehouse_migration(connection, ddl_observer=None):
    ...
    timestamp = _now()
    connection.execute(
        "INSERT OR IGNORE INTO erp_warehouses(code,name,normalized_name,is_active,created_at,updated_at) "
        "VALUES (?,?,?,?,?,?)",
        (DEFAULT_WAREHOUSE_CODE, DEFAULT_WAREHOUSE_NAME,
         DEFAULT_WAREHOUSE_NAME.casefold(), 1, timestamp, timestamp),
    )
    connection.execute(
        "INSERT OR IGNORE INTO erp_warehouses(code,name,normalized_name,is_active,created_at,updated_at) "
        "VALUES (?,?,?,?,?,?)",
        (SECONDARY_WAREHOUSE_CODE, SECONDARY_WAREHOUSE_NAME,
         SECONDARY_WAREHOUSE_NAME.casefold(), 1, timestamp, timestamp),
    )
    default_id = _warehouse_id(connection, DEFAULT_WAREHOUSE_CODE)
